with source_data as (
    select
        ...
        target_code,
        description
    from {{ source('ods', 'ODS_TARGET') }}
),
ranked as (
    select
        *,
        row_number() over (
            partition by nullif(ltrim(rtrim(target_code)), '')
            order by _airbyte_extracted_at desc, _airbyte_raw_id desc
        ) as rn
    from source_data
)
select
    try_cast(nullif(ltrim(rtrim(target_code)), '') as int) as target_code,
    nullif(ltrim(rtrim(description)), '') as target_description,
